...
def fit_lmers(fg_epochs, LHS, RHSs, parallel=True, n_cores=4, save_as=None):
    ...
    for rhs in RHSs:
        fg_lmer = fg_epochs.lmer(LHS=LHS, RHS=rhs, 
...
def get_lmer_AICs(lmer_coefs):
    ...
    for time in AICs.index.get_level_values('Time').unique():
        for chan in AICs.index.get_level_values('channel').unique():
            ...
            AICs.loc[slicer, 'min_delta']  = (AICs.loc[slicer, 'AIC'] -
                                          min(AICs.loc[slicer, 'AIC']))
...
def plot_lmer_AICs(aics):
    ...
    f, axs = plt.subplots(len(models), 2,
    ...
    for i, m in enumerate(models):  
        ...
        for c in channels:
            min_deltas = (aics.loc[pd.IndexSlice[:, m, c],
                                  ['min_delta', 'has_warning']]
                          .reset_index('Time'))
            traces.plot(min_deltas['Time'], min_deltas['min_delta'],
                        label=c)
            warn_ma = np.ma.where(min_deltas['has_warning'] > 0)
            traces.scatter(min_deltas['Time'].iloc[warn_ma], 
        ...
        # heat map
        _min_deltas = (aics.loc[pd.IndexSlice[:, m, :],'min_delta']
                      .reset_index('model', drop=True)
                      .unstack('channel')
                      .astype(float))
    
    
        _has_warnings = (aics.loc[pd.IndexSlice[:, m, :],'has_warning']
                      .reset_index('model', drop=True)
                      .unstack('channel')
                      .astype(bool))
        ...
        im_ma = heatmap.pcolormesh(
            ...
            _has_warnings.T,  
            cmap=cmap_ma, norm=norm_ma)
        yloc = mpl.ticker.IndexLocator(base=1, offset=.5)
...
def plot_lmer_rERPs(LHS, lmer_coefs, alpha=0.05):
    ...
    for col in LHS:
        f, ax_coef = plt.subplots(len(coefs), 
                                  figsize=(18,12), 
                                  gridspec_kw = {'hspace': .3})
        if len(coefs) == 1:
            ax_coef = [ax_coef]
        ...
        for idx, coef in enumerate(coefs):
            ...
            fg_lmer_coef = (
                lmer_coefs
                .loc[pd.IndexSlice[:, :, coef], col]
                .unstack(level='key')
                .reset_index('Time'))
 
            # log scale DF
            fg_lmer_coef['log10DF'] = (fg_lmer_coef['DF']
            ...
            # lmer SEs
            fg_lmer_coef['mn+SE'] = (fg_lmer_coef['Estimate'] + 
                                     fg_lmer_coef['SE']).astype(float) 
            fg_lmer_coef['mn-SE'] = (fg_lmer_coef['Estimate'] - 
                                     fg_lmer_coef['SE']).astype(float)

            fg_lmer_coef.plot(x='Time', y='Estimate', ax=ax_coef[idx], 
                                 color='black', alpha=.5,
                         label=coef)

            ax_coef[idx].fill_between(x=fg_lmer_coef['Time'], 
